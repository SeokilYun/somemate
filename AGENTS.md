...
대화 캡처와 고민을 공유하면 성격이 다른 AI 친구 3명(긍정이·신중이·부정이)이 각자의 관점으로 해석해주는 관계 상담 웹앱. 연애뿐 아니라 친구·동료·가족 관계도 다룬다. 모바일 우선 반응형 웹앱.
...
### 3. 친구 초대
- 긍정이/신중이/부정이 3개 카드: 아바타, 이름, 성격 설명, 대표 한마디.
...
- 신규 방 최초 진입 시 3캐릭터 인사 1회 자동 출력(긍정이→신중이→부정이 순) + 업로드 안내 메시지. 재진입 시 인사 반복 금지, 마지막 메시지부터 이어서 표시.
