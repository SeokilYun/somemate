...
코드 스캐폴딩 전(README.md 기준), AGENTS.md·docs/API.md만 존재. 이 계획은 0에서 시작하는 것을 전제로 한다.
...
| 단계 | 목표 | 완료 기준 |
| --- | --- | --- |
| M0 | 로컬 개발 환경 | 서버 MySQL `DATABASE_URL` 연결 + `npx prisma migrate dev`로 스키마 반영, `npm run dev` 기동 |
| M1 | 인증 | 회원가입/로그인/로그아웃, 세션으로 보호된 API 접근 가능 |
| M2 | Partner 저장 | Partner 생성/수정 API, 소유권 검증 |
| M3 | Conversation/Message 저장 | 상담방 생성(+인사 메시지 저장은 B와 연결), 메시지 영속 저장, 재접속 시 히스토리 복원 |
| M4 | 이미지 저장 | 업로드/서빙 API, 사용자 경로 격리 + 소유자 검증 |
| M5 | 후속 알림 | 구독 저장, 예약/취소, 24시간 디스패치 |
| M6 | 배포·운영 | 배포 파이프라인, 에러/비용 모니터링 |
...
- 최초 진입 인사 메시지 저장(1회) 로직: "이미 인사 메시지가 있는가" 판단은 저장된 Message 존재 여부로 처리 → 재진입 시 중복 생성 방지
- 재로그인/새로고침 후 메시지 순서·내용 복원 검증
